...
import {
  ...
  clearApplyState,
  selectJobApplicationApplying,
  selectJobApplicationApplyError,
  selectJobApplicationApplySuccess,
...
const JobDetailPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.gdprConsent) {
      toast.error("Lütfen aydınlatma metnini onaylayın");
      return;
    }
    ...
    try {
      ...
      // Toast message is handled by state effect.
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-canvas/50 py-16">
      <div className="max-w-4xl mx-auto px-4">
        {job ? (
          <div className="bg-white rounded-xl shadow p-6">
            ...
            {job.requirements && (
              <div className="mb-4 whitespace-pre-line">
                <h3 className="font-semibold text-ink">Aranan Nitelikler</h3>
            ...
            {job.benefits && (
              <div className="mb-6 whitespace-pre-line">
                <h3 className="font-semibold text-ink">Sunduğumuz İmkânlar</h3>
                <p className="text-gray-700">{job.benefits}</p>
              </div>
            )}

            <h2 className="text-2xl font-bold text-ink mb-4">Başvuru Formu</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm text-gray-700 mb-1">
                    Özgeçmiş (PDF/DOC/DOCX - en fazla 20MB)
                  </label>
                  <div className="flex items-center gap-3">
                    ...
                    <label
                      htmlFor="resumeFile"
                      className="inline-flex items-center bg-brand-red hover:bg-brand-red-deep text-white px-4 py-2 rounded cursor-pointer"
                    >
                      Dosya Seç
                    </label>
                    <span className="text-gray-600 text-sm truncate max-w-[200px]">
                      {form.resumeFile ? form.resumeFile.name : "Dosya seçilmedi"}
                    </span>
                    {form.resumeFile && (
                      <button
                        type="button"
                        onClick={() => setForm({ ...form, resumeFile: null })}
              ...
              <div>
                <button
                  type="submit"
                  disabled={applying}
                  className="bg-brand-red hover:bg-brand-red-deep text-white px-6 py-2 rounded"
                >
          ...
          <div className="text-center text-gray-600">İlan detayı yükleniyor...</div>
        )}
      </div>
    </div>
  );
