  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/admin/jobs/new" className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg">Yeni İlan</Link>
      </div>
      <div className="bg-gray-800 rounded-lg p-4 mb-6">
        <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Ara (başlık/bölüm/lokasyon)" className="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white" />
      </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  <th className="py-3 px-4 text-left text-gray-300">Bölüm</th>
                  <th className="py-3 px-4 text-left text-gray-300">Lokasyon</th>
              ...
              <tbody className="divide-y divide-gray-700">
                {filtered.map((j) => (
                  <tr key={j.id} className="hover:bg-gray-750">
                    ...
                    <td className="py-3 px-4 text-gray-300 text-xs">{j.employmentType}</td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs ${j.active ? 'bg-emerald-500/20 text-emerald-500' : 'bg-red-500/20 text-red-500'}`}>{j.active ? 'Aktif' : 'Pasif'}</span>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex gap-2">
                        <button onClick={()=>toggleActive(j)} className="px-2 py-1 rounded bg-yellow-600/20 text-yellow-400 text-xs">{j.active ? 'Pasifleştir' : 'Aktifleştir'}</button>
                        <button onClick={()=>navigate(`/admin/jobs/edit/${j.id}`)} className="px-2 py-1 rounded bg-blue-600/20 text-blue-400 text-xs">Düzenle</button>
                        <button onClick={()=>handleDelete(j.id)} className="px-2 py-1 rounded bg-red-700/20 text-red-400 text-xs">Sil</button>
                      </div>
                    </td>
                  </tr>
                ))}
