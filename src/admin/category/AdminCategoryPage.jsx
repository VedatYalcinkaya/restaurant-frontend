  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/admin/categories/new" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center transition-colors">
          <IconPlus className="h-5 w-5 mr-2" /> Yeni Kategori
        </Link>
      </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  <th className="py-3 px-4 text-left font-medium text-gray-300">Açıklama</th>
                  <th className="py-3 px-4 text-left font-medium text-gray-300">Sıra</th>
                ...
                {filtered.map((c) => (
                  <tr key={c.id} className="hover:bg-gray-750">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs ${c.active ? 'bg-emerald-500/20 text-emerald-500' : 'bg-red-500/20 text-red-500'}`}>
                        {c.active ? 'Aktif' : 'Pasif'}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleToggleActive(c)} className="p-1.5 rounded-lg bg-yellow-500/20 text-yellow-500 hover:bg-yellow-500/30" title="Durum Değiştir">
                          {c.active ? <IconToggleLeft className="h-5 w-5" /> : <IconToggleRight className="h-5 w-5" />}
                        </button>
                        <button onClick={() => navigate(`/admin/categories/edit/${c.id}`)} className="p-1.5 rounded-lg bg-blue-500/20 text-blue-500 hover:bg-blue-500/30" title="Düzenle">
                          <IconEdit className="h-5 w-5" />
                        </button>
                        <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg bg-red-500/20 text-red-500 hover:bg-red-500/30" title="Sil">
